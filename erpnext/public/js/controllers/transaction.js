...
erpnext.TransactionController = class TransactionController extends erpnext.taxes_and_totals {
	...
	qty_after_batch_select(doc, cdt, cdn) {
		frappe.model.trigger('qty', undefined, locals[cdt][cdn], false)
	}

	qty(doc, cdt, cdn) {
		let item = frappe.get_doc(cdt, cdn);
		if (item.cross_border_added) {
			return;
		}
	...
	get_and_set_item_details(doc, cdt, cdn) {
		...
		if (item.qty == 0){ //Mitch Added
			//Skip
			console.log("Skipping 0 qty")
		} else if(item.item_code || item.barcode || item.serial_no) {
	...
	apply_rule_on_other_items(args) {
		...
		for(var k in args) {
			...
			me.frm.doc.backorder_items.forEach(item => {
				var test = data
				if(item["ignore_pricing_rules"] != 1 && item["is_free_item"] != 1){
					if(!me.set_items.includes(item.name) && data.apply_rule_on_other_items.includes(item[data.apply_rule_on])){
						me.get_and_set_item_details(me.frm.doc, item.doctype, item.name)
		...
		function apply_discount_once(data, me) {
			...
			free_item_data.forEach(pr_row => {
				...
				for (let index = 0; index < rows_to_modify.length; index++) {
					...
					me.frm.script_manager.copy_from_first_row("items", row_to_modify, ["expense_account", "income_account"]);
