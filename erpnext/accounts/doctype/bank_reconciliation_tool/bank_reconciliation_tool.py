...
def auto_reconcile_vouchers(
	...
	for transaction in bank_transactions:
		linked_payments = get_linked_payments(
			transaction.name,
			["payment_entry", "journal_entry", "payment_group"],
			from_date,
			to_date,
...
def get_pg_matching_query(
	...
	amount_rank = frappe.qb.terms.Case().when(amount_equality, 1).else_(0)
	amount_condition = amount_equality if exact_match else pg.total > 0.0

	filter_by_date = pg.date.between(from_date, to_date)
